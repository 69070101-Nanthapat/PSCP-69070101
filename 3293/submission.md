...
```text
step1 รับค่าคำแรกเข้ามาก่อนแล้วตั้งให้เป็นคำที่ยาวที่สุด
step2 loop รับตัวที่เหลือและเช็คว่ายาวกว่าตัวก่อนหน้ามั้ย
step3 append คำที่inputมาเข้าไปใน list
step4 print "*" * len(คำที่ยาวที่สุด+4)
step5 สร้าง loop  print(f"* {x[i]} {" "*(most-len(x[i]))}*")
step4 print "*" * len(คำที่ยาวที่สุด+4)
```
