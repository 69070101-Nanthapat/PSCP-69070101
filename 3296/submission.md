...
```text
input รับเลข 3 เลขมา 2 บรรทัด
output (เลขตัวทที่ 1 - 3 ของบรรทัดบน+บรรทัดล่าง)//2 ตามลำดับ
```
...
```text
step1 x1, y1, z1 = map(int, input().split()) รับบรรทัดแรก
step2 x2, y2, z2 = map(int, input().split()) รับบรรทัดสอง
step3 print(f"{(x1 + x2) // 2} {(y1 + y2) // 2} {(z1 + z2) // 2}") ปริ้นตัวที่ 1,2,3 ของสองบรรทัดบวกกัน // 2
```
